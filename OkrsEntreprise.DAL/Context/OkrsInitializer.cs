...
{
    public class OkrsInitializer : System.Data.Entity.DropCreateDatabaseIfModelChanges<OkrsContext>
    {
        private Goal _goal1;
        private Goal _goal2;
        private Goal _goal3;
        ...
        protected override void Seed(OkrsContext context)
        {
            ...
            #region GoalIndicators
            InitializeGoalIndicators(context);
            #endregion
        ...
        private void InitializeGoalIndicators(OkrsContext context)
        {
            var meetsDeadlinesIndicatorValues = new List<IndicatorValue>()
            {
                new IndicatorValue(){Text = "Yes"},
                new IndicatorValue(){Text = "No"},
            };

            var meetDeadlineindicator = new Indicator()
            {
                Title = "Meets Deadlines",
                TenantId = 1,
                IndicatorValues = meetsDeadlinesIndicatorValues
            };

            var resultsIndicatorValues = new List<IndicatorValue>()
            {
                new IndicatorValue(){Text = "Poor"},
                new IndicatorValue(){Text = "Average"},
                new IndicatorValue(){Text = "Good"},
                new IndicatorValue(){Text = "Excellent"},
            };

            var resultsIndicator = new Indicator()
            {
                Title = "Results",
                TenantId = 1,
                IndicatorValues = resultsIndicatorValues,
                
            };

            context.Indicators.Add(meetDeadlineindicator);
            context.Indicators.Add(resultsIndicator);
            context.SaveChanges();


            var goal1Indicator1 = new GoalIndicator()
            {
                TenantId = 1,
                GoalId = _goal1.Id,
                IndicatorId = 1,
            ...
            var goal1Indicator2 = new GoalIndicator()
            {
                TenantId = 1,
                GoalId = _goal1.Id,
                IndicatorId = 2,
            ...
            context.GoalIndicator.Add(goal1Indicator1);
            context.GoalIndicator.Add(goal1Indicator2);
            context.SaveChanges();
        }
