okrsapp.controller('editgoal', ['$scope', '$location', '$timeout', '$routeParams', '$sce', 'localdata', 'dataservice', 'indicatorservice', function ($scope, $location, $timeout, $routeParams, $sce, localdata, dataservice, indicatorservice) {

    $scope.editformData = {};
    $scope.goalcatList = [];
    $scope.goalindicators = [];
    ...
    $scope.Init = function () {
        ...
        $scope.getgoal($routeParams.goal);
        $scope.getgoals();
    ...
    $scope.getgoal = function (id) {
        ...
        promise
          .success(function (data) {
              ...
              $scope.trustedDescription = $sce.trustAsHtml($scope.editformData.description);
    ...
    $scope.UpdateParent = function () {
        ...
        //    $scope.editformData.parent = newp;
        //}
        console.log($scope.editformData.parent);
        $scope.loadingImage = true;
        var promise = dataservice.editparent($scope.editformData);
        promise
          .success(function (data) {
    ...
    $scope.updatetitle = function (goaltitle) {
        $scope.editformData.title = goaltitle;
        $scope.loadingImage = true;
    ...
    $scope.getheadertemp = function () {
        ...
        else if ($scope.edthdr == 'prio') return 'edtprio';
    }
    ...
    $scope.editPriority = function () {
        $scope.oldPriority = $scope.editformData.priority;
        $scope.edthdr = 'prio';
    }


    $scope.resetPriority = function () {
        $scope.editformData.priority = $scope.oldPriority;
        $scope.oldPriority = 0;
        $scope.edthdr = 'h';
    }


    $scope.priorityOnChange = function (val) {

        var promise = dataservice.editgoalpriority($scope.editformData.id, val);
        promise
          .success(function (data) {

              $scope.editformData.priority = val;
              $scope.oldPriority = 0;
              $scope.edthdr = 'h';
          })
         .error(function (data, status, headers, config) {
    ...
    $scope.editdest = function () {
        ...
        $timeout(function () {
            $(".summernote").code($scope.editformData.description);
        }
            , 500);
    ...
    $scope.updatedes = function () {
        var textFromEditor = $('.summernote').code();
        if (textFromEditor.length > 1000) {
            alert("Description - Only 1000 characters allowed");
            return;
        }
        $scope.editformData.description = textFromEditor;
        $scope.loadingImage = true;
    ...
    $scope.updatecomment = function (indx, comment) {
        $scope.selcom = $scope.editformData.comments[indx];
        $scope.selcom.text = comment;
    ...
    $scope.getcomtemp = function (com) {
        //if ($scope.selcom.id == com.id) return 'editcomment'; else
            return 'discomment';
    }
    ...
    $scope.updatekr = function (kr, idx) {
        $scope.selkr = $scope.editformData.keyresults[idx];
        $scope.selkr.title = kr;
    ...
    $scope.editisopen = function () {
        swal({
            title: "Do you want to close this Objectives?",
            text: "The objective will be moved to closed objective list. You can at any time recover it.",
            type: "warning",
            showCancelButton: true,
            confirmButtonColor: "#DD6B55",
    ...
    $scope.refreshindicatorpanel = function () {

        for (var i = 0; i < $scope.indicatorList.length; i++) {
            var indicatorvalue =
                $scope.editformData.GoalIndicators[i] == null || $scope.editformData.GoalIndicators[i].ValueText == null
                    ? null : $scope.editformData.GoalIndicators[i].ValueText;

            $('#inline-indicator-' + i).editable({
                prepend: indicatorvalue,
                mode: 'inline',
                source: $scope.indicatorList[i].IndicatorValues,
                display: function (value, sourceData) {
                    ...
                    if (elem.length) {
                        ...
                        var promise = dataservice.editgoalindicator($scope.editformData.id, $scope.indicatorList[i].Id, value);
                        promise
                            .success(function (data) {
    ...
    $scope.getIndicators = function () {
        var promise = indicatorservice.list();
        promise
            .success(function (data) {
                $scope.indicatorList = data;
            })
            .error(function (data, status, headers, config) {
                if (status == 400) {
                    $scope.loadingImage = false;
